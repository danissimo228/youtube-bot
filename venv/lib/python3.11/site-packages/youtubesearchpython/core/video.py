...
class VideoCore(RequestCore):
    ...
    def __getVideoComponent(self, mode: str) -> None:
        ...
        if mode in ['getInfo', None]:
            ...
            duration = None
            try:
                duratin = str(datetime.timedelta(
                    seconds=int(getValue(responseSource, ['videoDetails', 'lengthSeconds'])))
                )
            except Exception:
                pass
            ...
            try:
                publish_date = datetime.datetime.fromisoformat(
                    getValue(responseSource, ['microformat', 'playerMicroformatRenderer', 'publishDate'])
                ).strftime('%d.%m.%Y %H:%M')
            except Exception:
                pass
            ...
            uploadDate = ''
            try:
                uploadDate = datetime.datetime.fromisoformat(
                    getValue(responseSource, ['microformat', 'playerMicroformatRenderer', 'uploadDate'])
                ).strftime('%d.%m.%Y %H:%M')
            except Exception:
                pass
