...
class AiohttpSession(BaseSession):
    ...
    async def make_request(
        ...
        try:
            async with session.post(
                url, data=form, timeout=self.timeout if timeout is None else timeout, ssl=False
            ) as resp:
    ...
    async def stream_content(
        ...
        async with session.get(
            url, timeout=timeout, headers=headers, raise_for_status=raise_for_status, ssl=False
        ) as resp:
